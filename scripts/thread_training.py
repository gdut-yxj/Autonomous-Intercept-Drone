...
class TrainingThread(QtCore.QThread):
    ...
    def run(self):
        ...
        file_path = 'logs/' + self.project_name + '/' + now_string \
                    + '_' + self.cfg.get('options', 'dynamic_name') \
                    + '_' + self.cfg.get('options', 'policy_name') \
                    + '_' + self.cfg.get('options', 'algo')
        ...
        if algo == 'PPO':
            ...
            aciton_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=noise_sigma)
            model = SAC(
                policy_base,
                self.env,
                action_noise=aciton_noise,
                policy_kwargs=policy_kwargs,
