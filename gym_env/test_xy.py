...
class MultirotorDynamicsVel(object):
    def __init__(self, cfg) -> None:
        ...
        if self.navigation_3d:
            ...
            self.action_space = spaces.Box(
                low=np.array([self.v_x_min, self.v_y_min, -self.v_z_max, -self.yaw_rate_max_rad]),
                high=np.array([self.v_x_max, self.v_y_max, self.v_z_max, self.yaw_rate_max_rad]),
                dtype=np.float32)
        else:
            if self.using_velocity_state:
            ...
            self.action_space = spaces.Box(
                low=np.array([self.v_x_min, self.v_y_min, -self.yaw_rate_max_rad]),
                high=np.array([self.v_x_max, self.v_y_max, self.yaw_rate_max_rad]),
                dtype=np.float32)
    ...
    def set_action(self, action):
        self.v_x_sp = action[0] * 0.7
        self.v_y_sp = action[1] * 0.7
        self.yaw_rate_sp = action[-1] * 2
        if self.navigation_3d:
            self.v_z_sp = float(action[2])
        else:
            self.v_z_sp = 0

        self.client.simPause(False)
        if len(action) == 3:
            self.client.moveByVelocityZAsync(self.v_x_sp, self.v_y_sp, -self.start_position[2], self.dt,
                                             drivetrain=airsim.DrivetrainType.MaxDegreeOfFreedom,
                                             yaw_mode=airsim.YawMode(is_rate=True,
                                                                     yaw_or_rate=math.degrees(self.yaw_rate_sp))).join()
        elif len(action) == 4:
            self.client.moveByVelocityAsync(self.v_x_sp, self.v_y_sp, -self.v_z_sp, self.dt,
                                            drivetrain=airsim.DrivetrainType.MaxDegreeOfFreedom,
                                            yaw_mode=airsim.YawMode(is_rate=True,
                                                                    yaw_or_rate=math.degrees(self.yaw_rate_sp))).join()
    ...
    def _get_state_feature(self):
        ...
        linear_velocity_x = velocity[0]
        linear_velocity_y = velocity[1]
        linear_velocity_x_norm = (linear_velocity_x - self.v_x_min) / (self.v_x_max - self.v_x_min) * 255
        linear_velocity_y_norm = (linear_velocity_y - self.v_y_min) / (self.v_y_max - self.v_y_min) * 255
        ...
        self.state_raw = np.array([distance, relative_pose_z, math.degrees(
            relative_yaw), linear_velocity_x, linear_velocity_y, linear_velocity_z, math.degrees(velocity[2])])
        state_norm = np.array([distance_norm, vertical_distance_norm, relative_yaw_norm,
                               linear_velocity_x_norm, linear_velocity_y_norm, linear_velocity_z_norm, angular_velocity_norm])
    ...
    def get_velocity(self):
        ...
        velocity_x = linear_velocity.x_val
        velocity_y = linear_velocity.y_val
